...
from server.dental_environment import DentalAlignerEnvironment, StepwiseDentalEnvironment, _SESSIONS
...
_stepwise_env = StepwiseDentalEnvironment()
...
# ---------------------------------------------------------------------------
# Stepwise (24-step sequential) endpoints
# ---------------------------------------------------------------------------

@app.post('/reset_stepwise')
async def reset_stepwise(request: Dict[str, Any] = Body(default={})) -> Dict[str, Any]:
    """Reset a 24-step sequential episode. Supports real clinical data sources."""
    task_id = request.get('task_id', 'task_easy')
    seed = request.get('seed', None)
    episode_id = request.get('episode_id', None)
    source = request.get('source', 'synthetic')
    patient_path = request.get('patient_path', None)
    difficulty_params = request.get('difficulty_params', None)

    obs = _stepwise_env.reset(
...
@app.post('/step_stepwise')
async def step_stepwise(request: Dict[str, Any] = Body(...)) -> Dict[str, Any]:
    """Commit tooth poses for the next stage (1 of 24 sequential steps)."""
    episode_id = request.get('episode_id')
    poses = request.get('poses')
    if not episode_id or not poses:
        return {'error': 'Requires episode_id and poses (28x7 array)'}
    return _stepwise_env.step(episode_id, poses)


@app.post('/tool')
async def tool_call(request: Dict[str, Any] = Body(...)) -> Dict[str, Any]:
    """Execute a tool-use action in stepwise mode."""
    episode_id = request.get('episode_id')
    tool = request.get('tool')
    args = request.get('args', {})
    if not episode_id or not tool:
        return {'error': 'Requires episode_id and tool name'}
    return _stepwise_env.handle_tool(episode_id, tool, args)


@app.get('/datasets')
async def list_datasets():
    """List available real clinical dataset sources."""
    from server.dataset_loader import list_datasets as _list_ds
